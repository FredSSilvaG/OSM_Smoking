#New for smoking status
plot_relative_abundance <- function(data_species, metadata, level = "Phyla", 
  ...
  data_species <- data_species %>%
    separate(clade_name, into = c("Kingdom", "Phyla", "Class", "Order", "Family", "Genus", "Species"),
             sep = "\\|", fill = "right")
  
  # Remove prefixes like k__, p__, etc.
  data_species[] <- lapply(data_species, function(x) if (is.character(x)) gsub("^[kpcfogs]__", "", x) else x)
  
  # Clean Genus and Species names: remove underscores and italicize
  data_species <- data_species %>%
    mutate(
      Genus   = ifelse(!is.na(Genus), paste0("*", Genus, "*"), NA),
      Species = ifelse(!is.na(Species), gsub("_", " ", Species), NA)
    )
  
  # Long format abundance
  abundance_data <- data_species[, setdiff(names(data_species), c("Kingdom", "Class", "Order", "Family"))]
  abundance_data <- gather(abundance_data, samples, RA, -c("Phyla", "Genus", "Species"))
  ...
  abundance_data$Legend_Taxon <- ifelse(abundance_data$RA < low_abundance_threshold,
                                        ...
                                        paste0(gsub("_", " ", abundance_data[[level]])))
